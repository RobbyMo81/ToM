...
No autonomous strategy changes without human approval (nightly job only proposes)
6) What to delete vs keep (given your new requirements)
...
News/catalyst gathering uses Tier 2, then Tier 0 summarizes into your internal format.
3) Nightly pipeline (OpenClaw cron) — concrete job set
...
6) Next step (single-step loop, so we don’t drift)
...
Perplexity only for “news/catalyst” ingestion (and only for your sector/ticker universe)
5) “Eliminate noise” implementation checklist (what we do first)

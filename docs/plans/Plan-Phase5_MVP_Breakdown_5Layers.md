...
class TradingSafety {
  ...
  async validateProposal(proposal: TradeProposal): Promise<SafetyCheckResult> {
    ...
    if (positionPercent > this.config.maxPositionSizePercent) {
      violations.push(`Position ${positionPercent.toFixed(2)}% exceeds limit ${this.config.maxPositionSizePercent}%`);
    }
    ...
    if (sectorExposure + positionPercent > this.config.maxSectorConcentration) {
      violations.push(`Sector ${sector} would exceed ${this.config.maxSectorConcentration}% limit`);
    }
...
class ConfirmationWorkflow {
  ...
  private async waitForUIConfirmation(traderId: string): Promise<boolean> {
    ...
    const result = await Promise.race([this.pollConfirmation(traderId), this.delay(timeout).then(() => false)]);
...
class AlpacaIntegration {
  ...
  async getBars(symbol: string, timeframe: "1m" | "5m" | "15m" | "1h" | "1d", limit: number = 100): Promise<any[]> {
...
class PositionTracker {
  ...
  async getSectorExposure(): Promise<Map<string, number>> {
    ...
    for (const pos of positions) {
      ...
      exposure.set(pos.sector, (exposure.get(pos.sector) || 0) + pct);
    }

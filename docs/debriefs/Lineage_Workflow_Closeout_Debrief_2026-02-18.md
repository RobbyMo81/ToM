...
- Date: 2026-02-18
- Change: planning-mode alignment updated vector-memory policy to exclude root
  markdown (`*.md`) and automation SOP docs (`automation/**/*.md`).
- Files touched:
  - `src/integrations/knowledgeLoader.ts`
  - `README.md`
  - `.tom-workspace/whoiam.md`
- Runtime impact:
  - local ingestion now loads `docs/**/*.md` only
  - root markdown and automation docs are excluded from vector indexing
  - retrieval surface narrows to documentation corpus + web enrichment inputs
- Backward compatibility notes:
  - existing vector rows from previous policy are not automatically removed by
    this change
  - run a fresh local ingest cycle to converge active index state with policy

...
- They never talk to each other — but the error may look like auth failure, not connection failure, if something *else* is on 18789
...
### L7 — The error hint "set gateway.remote.token to match gateway.auth.token" is about the *client*, not the server

This hint appears in `formatGatewayAuthFailureMessage()` whenever a **CLI client** (`isGatewayCliClient = true`) sends the wrong token. It tells you what config key the *client* should set — not that the server is misconfigured. Do not interpret it as "the server's remote token is wrong."

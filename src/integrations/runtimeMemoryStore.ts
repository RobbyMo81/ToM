...
export interface LineageIdentitySummary {
  identityAgent: string;
  identityVersion: string;
  promptClass: string;
}

export interface LineageEventAnnotations {
  role: string | null;
  stage: string | null;
...
export interface LineageSummaryResponse {
  ...
  event: {
    ...
    message: string;
    createdAt: string;
    annotations: LineageEventAnnotations | null;
  } | null;
  identity: LineageIdentitySummary | null;
}
...
export class RuntimeMemoryStore {
  ...
  getLatestLineageSummary(): LineageSummaryResponse {
    ...
    const identity = this.readLineageIdentity(context);
    ...
    const eventRow = this.db
      .prepare(
        `
      SELECT id, event_type, event_level, message, payload_json, created_at
      FROM task_events
    ...
    return {
      ...
      event: eventRow
        ? {
            ...
            annotations: this.readEventAnnotations(eventRow.payload_json),
          }
        : null,
      identity,
    };
  ...
  getLineageRuns(limit = 20, filters: LineageRunsFilter = {}): LineageRunsResponse {
    ...
    const latestEventStmt = this.db.prepare(
      "SELECT event_type, event_level, message, payload_json, created_at FROM task_events WHERE workflow_run_id = ? ORDER BY created_at DESC LIMIT 1"
    );
    ...
    const runs: LineageRunHistoryItem[] = pageRows.map((row) => {
      ...
      const deployStatus = proposalId
        ...
        : null;
      const latestEventRow = latestEventStmt.get(row.id) as
        | {
            event_type: string;
            event_level: string;
            message: string;
      ...
      return {
        ...
        latestEvent: latestEventRow
          ? {
              ...
              eventLevel: latestEventRow.event_level,
              message: latestEventRow.message,
              createdAt: latestEventRow.created_at,
              annotations: this.readEventAnnotations(latestEventRow.payload_json),
            }
          : null,
      };
    });
  ...
  private readLineageIdentity(context: Record<string, unknown>): LineageIdentitySummary | null {
    const identity = this.safeReadRecord(context.identity);
    const identityAgent = typeof identity.identityAgent === "string" ? identity.identityAgent : null;
    const identityVersion = typeof identity.identityVersion === "string" ? identity.identityVersion : null;
    const promptClass = typeof identity.promptClass === "string" ? identity.promptClass : null;

    if (!identityAgent || !identityVersion || !promptClass) {
      return null;
    }
  ...
  private readEventAnnotations(payloadJson: string): LineageEventAnnotations | null {
    const payload = this.safeParseJson(payloadJson);
    const role = typeof payload.role === "string" ? payload.role : null;
    const stage = typeof payload.stage === "string" ? payload.stage : null;
    const authority = typeof payload.authority === "string" ? payload.authority : null;
    const policyDecision = typeof payload.policyDecision === "string" ? payload.policyDecision : null;
    const decisionRationale = typeof payload.decisionRationale === "string" ? payload.decisionRationale : null;

    if (!role && !stage && !authority && !policyDecision && !decisionRationale) {
      return null;
    }
